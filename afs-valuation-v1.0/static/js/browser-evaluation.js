...
async function afsEvaluateSingleRow(row, options, spreadsheet, mappings, feedbackCtx = '', onStep = null) {
    ...
    let modelUsedVision = null;
    let modelUsedMarket = null;
    const evalStartedAt = Date.now();

    const imageUrls = afsCollectImageUrls(resolvedRow, mappings, spreadsheet);
    let visionImagesSent = 0;

    const evalModel = options.model || 'gemini-1.5-flash';

    const recordModelUsage = (res, phaseLabel, itemPercent) => {
        if (!res) return;
        const used = res.apiModel || res.fallbackModel;
        if (phaseLabel.includes('Vision')) modelUsedVision = used;
        else modelUsedMarket = used;
        const warn = typeof afsBuildModelFallbackWarning === 'function'
            ? afsBuildModelFallbackWarning(res, phaseLabel)
            : null;
        if (warn) {
            modelFallbacks.push(warn);
            emit(`⚠ ${warn}`, itemPercent);
        }
    };
    ...
    if (options.runTag || options.runAge || options.runConservation || imageUrls.length) {
        ...
        if (loadedParts > 0) {
            ...
            try {
                const prep = typeof afsPrepareCachedGeminiCall === 'function'
                    ? await afsPrepareCachedGeminiCall(s.api_key, evalModel, 'vision', AFS_VISION_PROMPT, imageParts)
                    : { parts: [{ text: AFS_VISION_PROMPT }, ...imageParts], opts: {} };
                const res = await afsGeminiRequest(s.api_key, evalModel, prep.parts, true, prep.opts);
                recordModelUsage(res, 'Vision', 40);
    ...
    if (options.runMarket || options.runCategoria || options.runAtivo) {
        ...
        const prep = typeof afsPrepareCachedGeminiCall === 'function'
            ? await afsPrepareCachedGeminiCall(s.api_key, evalModel, 'market', AFS_VALUATION_PROMPT, [{ text: dynamicText }])
            : { parts: [{ text: `${AFS_VALUATION_PROMPT}${dynamicText}` }], opts: {} };
        const res = await afsGeminiRequest(s.api_key, evalModel, prep.parts, true, prep.opts);
        recordModelUsage(res, 'Mercado', 80);
    ...
    const evaluation = {
        ...
        model_requested: evalModel,
        model_used_vision: modelUsedVision,
        model_used_market: modelUsedMarket,
        model_fallbacks: modelFallbacks.length ? [...modelFallbacks] : undefined,
        model_fallback_warning: modelFallbacks.length ? modelFallbacks.join(' | ') : undefined,
        created_at: new Date().toISOString()
    };
...
async function browserRunEvaluation(options, onProgress) {
    ...
    for (const row of rows) {
        ...
        try {
            ...
            if (result.model_fallback_warning) {
                sessionModelFallbacks.push({
                    row: rowIdx,
                    control: controlVal,
                    warnings: result.model_fallbacks || [result.model_fallback_warning]
                });
            }
            ...
            onProgress({
                ...
                model_used_vision: result.model_used_vision,
                model_used_market: result.model_used_market,
                model_fallback_warning: result.model_fallback_warning,
                model_fallbacks: result.model_fallbacks,
                stepLabel: result.model_fallback_warning
                    ? `Item ${itemNum}/${totalPending}: concluído com fallback de modelo ⚠`
                    : `Item ${itemNum}/${totalPending}: avaliação concluída ✓`,
                overallPercent: Math.round((completedCount / totalPending) * 100),
                itemPercent: 100
            });
        } catch (e) {
    ...
    const finishedMessage = sessionModelFallbacks.length
        ? `${sessionModelFallbacks.length} item(ns) usaram modelo alternativo (fallback) — tokens podem custar mais que o selecionado.`
        : '';
    onProgress({
        status: 'finished',
        overallPercent: 100,
        stepLabel: 'Avaliação finalizada.',
        message: finishedMessage,
        session_model_fallbacks: sessionModelFallbacks,
        session_model_fallback_count: sessionModelFallbacks.length
    });
}
