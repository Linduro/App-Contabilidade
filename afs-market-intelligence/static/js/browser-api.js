...
window.AFSMarketAPI = {
  ...
  /** Sem dados demo — use importação real ou prospecção RF. */
  seedDemo: async function () {
    if (!auth.currentUser) throw new Error('Faça login antes de importar leads.');
    return { status: 'ok', count: 0, message: 'Dados fictícios desativados.' };
  },
};
