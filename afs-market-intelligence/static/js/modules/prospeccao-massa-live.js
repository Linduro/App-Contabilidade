...
import {
  fetchRfStatus, startRfIngest, exportProspectosExcel,
  fetchProspectDefaults, mapProspectoToStore, backendConfigHint, getHttpApiBase,
  PROSPECT_EXPORT_COLS,
} from '../adapters/rf-pipeline-api.js';
import { exportExcel } from '../components/export.js';

/** Import dinâmico — evita conflito pollJob em cache antigo. */
async function pollRfJob(jobId, onTick) {
  const v = window.__AFS_BUILD__ || Date.now();
  const mod = await import('../adapters/rf-pipeline-api.js?b=' + encodeURIComponent(v));
  return mod.pollJob(jobId, onTick);
}
...
async function refreshResults(root) {
  ...
  try {
    ...
    const rows = (search.rows || []).filter(function (r) { return !isFictitiousCompany(r); });
    if (rows.length !== (search.rows || []).length) purgeAllProspectDemoStorage();
    state.counts = {
      todas: rows.length,
      nao_enriquecidas: rows.filter(function (r) { return !r.enriquecida; }).length,
      enriquecidas: rows.filter(function (r) { return r.enriquecida; }).length,
      novas: counts.novas || 0,
    };
    state.rows = rows;
...
function renderResultsArea(root) {
  ...
  if (!state.rows.length || state.rows.some(isFictitiousCompany)) {
    if (state.rows.some(isFictitiousCompany)) {
      purgeAllProspectDemoStorage();
      state.rows = [];
      state.total = 0;
      state.counts = { todas: 0, nao_enriquecidas: 0, enriquecidas: 0, novas: 0 };
...
function importEmpresasCrm(empresas) {
  const rows = (empresas || []).filter(function (e) {
    return e && e.cnpj_basico && e.enriquecimento_status !== 'error' && !isFictitiousCompany(e);
  });
...
async function renderAdminPanel(el) {
  ...
  el.querySelector('#adm-rf-start')?.addEventListener('click', async function () {
    ...
    try {
      ...
      await pollRfJob(res.job_id, function (j) { log.textContent = j.status + ' · ' + (j.message || ''); });
  ...
  el.querySelector('#adm-social-run')?.addEventListener('click', async function () {
    ...
    try {
      ...
      if (res.job_id) {
        ...
        await pollRfJob(res.job_id, function (j) {
