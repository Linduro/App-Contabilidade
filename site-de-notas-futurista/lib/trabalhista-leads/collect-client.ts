...
import { DATAJUD_SEARCH_DAYS, triageTrabalhista } from "@/lib/datajud/triage"
import {
...
export async function runTrabalhistaCollectInBrowser(
  ...
  const candidates: NonNullable<ReturnType<typeof triageTrabalhista>>[number][] = []
  ...
  for (const trt of trts) {
    ...
    const triaged = triageTrabalhista(sources, trt, params.natureza)
  ...
  stats.mensagem =
    stats.novos > 0
      ? `${stats.consultados} no Datajud (últimos 2 meses), ${stats.novos} novo(s) lead(s) após triagem.`
      : stats.analisados > 0
        ? `${stats.consultados} no Datajud, ${stats.analisados} na triagem — nenhum novo (já existiam).`
        : stats.consultados > 0
          ? `${stats.consultados} processo(s) nos últimos 2 meses — nenhum passou na triagem ou filtros regionais.`
          : "Nenhum processo no Datajud nos últimos 2 meses. Verifique conexão ou filtros regionais."
