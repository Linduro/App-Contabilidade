import { z } from "zod"
import { applyNetworkingDevDefaults } from "./dev-defaults.js"

const envSchema = z
  .object({
    DATABASE_URL: z.string().min(1),
    REDIS_URL: z.string().min(1),
    STORAGE_MODE: z.enum(["pglite", "postgres"]).optional(),
    NETWORKING_LOCAL: z.enum(["true", "false"]).optional(),
    EMBEDDING_PROVIDER: z.enum(["openai", "google", "mock"]).default("mock"),
    ...
    WEB3_ENABLED: z.enum(["true", "false"]).optional(),
...
export function isInlineQueue(): boolean {
  return process.env.REDIS_URL === "inline" || process.env.NETWORKING_LOCAL === "true"
}
