...
async function testApiKeys() {
    ...
    try {
        const model = document.getElementById('aiModelSelect')?.value || 'gemini-1.5-flash';
        const data = await apiFetch('/api/test-keys', { method: 'POST', body: JSON.stringify({ model }) });
        ...
        for (const [api, result] of Object.entries(data)) {
            ...
            if (result.model_fallback && result.message) {
                logModelFallback(result.message, { source: 'test-keys' });
            }
        }
    } catch (e) {
...
function showAlert(message, type = 'info', options = {}) {
    ...
    if (options.modelFallback) alert.classList.add('alert-model-fallback');
    alert.innerHTML = `<span>${icons[type] || ''}</span> ${message}`;

    if (!options.keepPrevious) {
        container.querySelectorAll(`.alert-${type}`).forEach(a => {
            if (!a.classList.contains('alert-model-fallback')) a.remove();
        });
    }
...
function clearModelFallbackBanner() {
    window.__afs_model_fallback_log = [];
    const el = document.getElementById('evalModelFallbackBanner');
    if (el) {
...
function logModelFallback(warning, meta = {}) {
    if (!warning) return;
    window.__afs_model_fallback_log = window.__afs_model_fallback_log || [];
    window.__afs_model_fallback_log.push({ warning, ...meta, at: new Date().toISOString() });
    renderModelFallbackBanner();
    showAlert(warning, 'warning', { modelFallback: true, keepPrevious: true, duration: 15000 });
}

function renderModelFallbackBanner() {
    const el = document.getElementById('evalModelFallbackBanner');
    if (!el) return;
    const log = window.__afs_model_fallback_log || [];
    if (!log.length) {
        el.style.display = 'none';
        el.innerHTML = '';
        return;
    ...
    const items = log.slice(-8).map(entry => {
        const ctrl = entry.control != null ? ` controle ${entry.control}` : '';
        const row = entry.row != null ? ` linha ${entry.row}` : '';
        return `<li>${entry.warning}${ctrl || row ? ` (${[ctrl, row].filter(Boolean).join(', ')})` : ''}</li>`;
    });
    el.innerHTML = `
        <strong>⚠ Modelo alternativo (fallback) — custo de tokens pode ser maior</strong>
        <ul>${items.join('')}</ul>
        <small>Selecionado no dropdown ≠ modelo que a API aceitou. Para menor custo, prefira Gemini 1.5 Flash e confira o teste de conexão.</small>
    `;
}
...
function updateEvalProgressUI(data) {
    ...
    if (detail) {
        ...
        if (data.model_used_vision && data.model_requested) {
            const reqLabel = typeof afsGeminiModelLabel === 'function'
                ? afsGeminiModelLabel(data.model_requested)
                : data.model_requested;
            const visionOk = !data.model_fallbacks?.some(w => w.includes('Vision'));
            if (!visionOk && data.model_used_vision) {
                parts.push(`Vision: ${data.model_used_vision}`);
            }
        }
        if (data.model_used_market && data.model_fallbacks?.some(w => w.includes('Mercado'))) {
            parts.push(`Mercado: ${data.model_used_market}`);
        }
...
function processEvaluationEvent(data, counters, pendingAtStart) {
    ...
    if (data.status === 'finished') {
        ...
        if (data.session_model_fallback_count > 0) {
            showAlert(
                `${data.session_model_fallback_count} item(ns) com fallback de modelo. ${data.message || 'Tokens podem custar mais que o modelo selecionado.'}`,
                'warning',
                { modelFallback: true, keepPrevious: true, duration: 20000 }
            );
        } else {
    ...
    if (data.model_fallback_warning) {
        logModelFallback(data.model_fallback_warning, { row: data.row, control: data.control });
    }
